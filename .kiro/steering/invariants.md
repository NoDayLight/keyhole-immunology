...
6. **One offline report.** The report is one self-contained HTML target (approximately 3–6 MB with production assets), with JavaScript and structures vendored inline. It uses no CDN, network request, credential, or Node toolchain. `report.py` concatenates plain IIFE modules.
