...
6. **One offline report.** The report is one self-contained HTML target (approximately 2.0–2.4 MB with compact display-only structure payloads, an inlined pinned WebGL engine, and an embedded font subset), with CSS, JavaScript, structures, and third-party runtime inlined. It uses no CDN, runtime import, remote texture, external font, network request, credential, `eval`, or Node toolchain. `report.py` concatenates plain IIFE modules and one inline module element for the vendored engine.
9. **Byte-exact third-party runtime.** Browser dependencies are official exact-pinned upstream files copied byte-for-byte under `resources/vendor`, with license text and a SHA-256 verified on every render. Module syntax is adapted only by anchored transformations that fail loudly; a vendored file is never rewritten in place. Every artifact's upstream URL, integrity value, digest, license, and pin reason is recorded in `resources/vendor/PROVENANCE.md`.
